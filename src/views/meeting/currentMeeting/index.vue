...
import { getCurrentPageList, cancel, oneClickCancelRepetitionMeeting } from '@/api/currentMeeting'
...
export default {
  ...
  mounted() {
    const { meetingId, type } = this.$route.query
    if (meetingId && type && type === 'edit') {
      this.detailsId = meetingId
      ...
      this.$confirm('是否取消该会议?', '提示', {
        ...
        .then(() => {
          ...
          if ([0, 1].includes(row.scheduleType)) {
            ...
            this.repetitionCancelFun(row, '1')
          }
        })
        .catch(() => {})
    },
    repetitionCancelFun(row, type) {
      oneClickCancelRepetitionMeeting({ meetingId: row.id, repetitionId: row.meetingRepetitionId })
        .then((res) => {
          this.$message.success(type === '1' ? '取消成功！' : '结束成功！')
          this.query()
        })
        .catch(() => {
          this.loading = false
      ...
      this.$confirm('是否结束该会议?', '提示', {
        ...
        .then(() => {
          ...
          if ([0, 1].includes(row.scheduleType)) {
            ...
            this.repetitionCancelFun(row, '2')
          }
        })
        .catch(() => {})
    },
